...
public class DatabaseConnection {
    ...
    private static void insertSampleData(Statement stmt) throws SQLException {
        ...
        // Clear any existing data - be defensive in case tables are empty/new
        try {
            stmt.execute("DELETE FROM transactions");
            stmt.execute("DELETE FROM accounts");
            stmt.execute("DELETE FROM customers");
        } catch (SQLException e) {
            // If deletes fail (e.g. tables not fully created yet), log and continue
            System.out.println("⚠️  Could not clear existing data (may be first-time init): " + e.getMessage());
        }
    ...
    public static String getDatabaseStats() {
        try (Connection conn = getConnection();
            ...
            try (ResultSet rs1 = stmt.executeQuery("SELECT COUNT(*) FROM customers")) {
                if (rs1.next()) customerCount = rs1.getInt(1);
            }

            try (ResultSet rs2 = stmt.executeQuery("SELECT COUNT(*), SUM(balance) FROM accounts")) {
                if (rs2.next()) {
            ...
            double avgBalance = accountCount > 0 ? totalBalance / accountCount : 0.0;
            ...
            return String.format("""
                ...
                """, customerCount, accountCount, totalBalance, avgBalance);
